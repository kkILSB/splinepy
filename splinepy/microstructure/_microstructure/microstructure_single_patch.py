...
class _MicrostructureSinglePatch(_SplinepyBase):
    ...
    def __init__(
        ...
        single-patch deformation functions.

        Parameters
        ----------
        deformation_function : single-patch
          Outer function that describes the contour of the microstructured
          geometry
        tiling : int / list<int>
          microtiles per parametric dimension
        microtile : spline / list<spline> / user-object
    ...
    def deformation_function(self, deformation_function):
        ...
        microstructure. Must be single-patch type and as such inherit from
        splinepy.Spline.
    ...
    def tiling(self, tiling):
        ...
        if not isinstance(tiling, list) and not isinstance(tiling, int):
            raise ValueError(
                "Tiling mus be either list of integers or a integer"
            )
        self._tiling = tiling
    ...
    def parametrization_function(self, parametrization_function):
        ...
        None
        """
        if not ((parametrization_function is None) or
                callable(parametrization_function)): raise TypeError(
                        "parametrization_function must be callable")
    ...
    def parameter_sensitivity_function(self, parameter_sensitivity_function):
        ...
        None
        """
        if not ((parameter_sensitivity_function is None) or
                callable(parameter_sensitivity_function)): raise TypeError(
                        "parametrization_function must be callable")
    ...
    def create(
        ...
        **kwargs,
        ...
        """Create a Microstructure for a single-patch deformation function.
        ...
        **kwargs
        ...
        self._microstructure_created = _Multipatch(splines=spline_list_ms)

        # Add fields if requested
        if macro_sensitivities or parameter_sensitivities:
            self._microstructure_created.add_fields(
                spline_list_derivs, self.deformation_function.dim
            )

        return self._microstructure_created
    ...
    def show(self, use_saved=False, **kwargs):
        ...
        **kwargs : kwargs
        ...
        # Show in vedo
        return _gus.show(
            ["Deformation Function", self._deformation_function],
